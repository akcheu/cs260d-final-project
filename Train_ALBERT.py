...
def train(model, train_loader, val_loader, optimizer):  
  ...
  for epoch in range(EPOCHS):
    ...
    total_train_loss = 0
    total_train_acc  = 0
    for batch_idx, (pair_token_ids, mask_ids, seg_ids, y, n) in enumerate(tqdm(train_loader, desc="Train")):
      optimizer.zero_grad()
    ...
    with torch.no_grad():
      for batch_idx, (pair_token_ids, mask_ids, seg_ids, y, n) in enumerate(tqdm(val_loader, desc="Val")):
        optimizer.zero_grad()
        pair_token_ids = pair_token_ids.to(device)
        mask_ids = mask_ids.to(device)
        seg_ids = seg_ids.to(device)
        ...
        for i in range(len(labels)):
          label_name = 'entailment' if labels[i] == 0 else 'contradiction' if labels[i] == 1 else 'neutral'
    ...
    plt.title('Albert - MNLI Class Accuracies', fontsize=15, fontweight='bold')
    plt.xlabel('Class', fontsize=12)
    plt.ylabel('Contains Negation Word (Spurious)', fontsize=12)
    plt.savefig(f'plots/Albert_{epoch}.png')
    print('\n')

  plt.figure()
  plt.plot(train_loss_values)
  plt.title('Albert - Train Loss', fontsize=15, fontweight='bold')
  plt.xlabel('Epoch', fontsize=12)
  plt.ylabel('Loss', fontsize=12)
  plt.savefig(f'plots/Albert_TrainLoss.png')

  plt.figure()
  plt.plot(val_loss_values)
  plt.title('Albert - Validation Loss', fontsize=15, fontweight='bold')
  plt.xlabel('Epoch', fontsize=12)
  plt.ylabel('Loss', fontsize=12)
  plt.savefig(f'plots/Albert_ValLoss.png')

  model.save_pretrained("Albert", from_pt=True)
